...
-- Resize the current window with CTRL+<arrows>. The arrows are otherwise free:
-- the maps above only disable the bare arrow keys, and <C-w>+/-/</> still work.
--
-- The window under the cursor grows toward the pressed direction and shrinks
-- away from it, so <C-Up> makes it taller and <C-Down> shorter regardless of
-- which neighbour actually gives up the space. Steps of 2 keep a held key
-- responsive without overshooting.
--
-- NOTE: a terminal has to send distinct keycodes for CTRL+arrow for these to
-- fire (kitty keyboard protocol, e.g. ghostty). Check with `:map <C-Up>` and,
-- if the terminal folds them into plain arrows, fall back to <C-w>+/-/</>.
vim.keymap.set("n", "<C-Up>", "<cmd>resize +2<CR>", { desc = "Increase window height" })
vim.keymap.set("n", "<C-Down>", "<cmd>resize -2<CR>", { desc = "Decrease window height" })
vim.keymap.set("n", "<C-Left>", "<cmd>vertical resize -2<CR>", { desc = "Decrease window width" })
vim.keymap.set("n", "<C-Right>", "<cmd>vertical resize +2<CR>", { desc = "Increase window width" })

-- Put every window back to an even share, the way a fresh split leaves them.
